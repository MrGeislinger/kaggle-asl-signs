...
print(f'{MODEL_DIR=}')
MODEL_NAME = MODEL_DIR.split('MODEL-')[-1]
...
if CFG.is_training:
    try:
        ...
        for i in tqdm(range(len(train))):
            ...
            data_resize = compress_frames(data_key_frames)
            X[i] = data_resize
            y[i] = train.iloc[i].label
...
def get_sequence_model(max_frames: int, num_features: int):
    ...
    ## lips
    p = tf.keras.layers.GRU(128, return_sequences=True)(
        input_lips,
    )
    p = tf.keras.layers.GRU(64)(p)

    #
    concat_hands = tf.keras.layers.Concatenate()([l, r, p])
    x = tf.keras.layers.Dense(256)(concat_hands)
...
model, history = run_experiment(
    ...
    model_path=f'{MODEL_DIR}/model_best_weights.h5',
    batch_size=BATCH_SIZE,
    epochs=EPOCHS,
    patience=PATIENCE,
)

model.save(f'{MODEL_DIR}/model-{model_details}.h5')
