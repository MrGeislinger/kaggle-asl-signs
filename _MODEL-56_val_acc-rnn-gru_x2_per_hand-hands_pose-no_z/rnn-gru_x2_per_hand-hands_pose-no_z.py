...
def get_sequence_model(max_frames: int, num_features: int):
    ...
    input_face = tf.keras.layers.Lambda(
        lambda x: x[:, :, START_FACE*N_DIMS:END_FACE*N_DIMS],
        output_shape=(MAX_FRAMES, (END_FACE - START_FACE), N_DIMS),
    )(frame_features_input)
    input_pose = tf.keras.layers.Lambda(
        lambda x: x[:, :, START_POSE*N_DIMS:END_POSE*N_DIMS],
        output_shape=(MAX_FRAMES, (END_POSE - START_POSE), N_DIMS),
    )(frame_features_input)


    face_avg = tf.keras.layers.Lambda(
        lambda x: tf.reduce_mean(x, axis=-1, keepdims=True)
    )(input_face)
    pose_avg = tf.keras.layers.Lambda(
        lambda x: tf.reduce_mean(x, axis=-1, keepdims=True)
    )(input_pose)

    lhand_face_pose = tf.keras.layers.Concatenate(axis=-1)([
        face_avg,
        pose_avg,
        input_lhand,
    ])
    # Why `mask`: https://keras.io/api/layers/recurrent_layers/gru/
    l = tf.keras.layers.GRU(128, return_sequences=True)(
        lhand_face_pose,
        mask=mask_input,
    )
    ...
    rhand_face_pose = tf.keras.layers.Concatenate(axis=-1)([
        face_avg,
        pose_avg,
        input_rhand,
    ])
    r = tf.keras.layers.GRU(128, return_sequences=True)(
        rhand_face_pose,
        mask=mask_input,
    )
